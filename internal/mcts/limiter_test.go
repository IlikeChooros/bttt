...
func TestLimiterSingleLimits(t *testing.T) {
	...
	if ok := limiter.Ok(1, 10); ok {
		t.Errorf("<Size=%d: ok=%v, want=%v", 10, ok, !ok)
	}

	if ok := limiter.Ok(99, 9); !ok {
...
func TestLimiterCombos(t *testing.T) {
	...
	if !(limiter.Ok(99, 10) && !limiter.Expand()) {
		t.Error(">Nodes+Memory failed: ok=", limiter.Ok(99, 10), "expand=", limiter.Expand())
	}
	if !(!limiter.Ok(101, 10) && !limiter.Expand()) {
		t.Error("<Nodes+Memory failed: ok=", limiter.Ok(101, 10), "expand=", limiter.Expand())
	}
	...
	if !(limiter.Ok(1, 100) && !limiter.Expand()) {
		t.Error(">Time+Memory failed: ok=", limiter.Ok(1, 100), "expand=", limiter.Expand())
	}
